...
exports.load = async(function* (req, res, next, id) {
  try {
    req.message = yield Message.load(id);
    if (!req.message) return next(new Error('Message not found'));
  } catch (err) {
    return next(err);
  }
  next();
...
exports.index = async(function* (req, res) {
  ...
  const articles = yield Message.list(options);
  const count = yield Message.count();
